...
def random_grid(n: int) -> np.array:
    return np.random.choice(vals, n*n, p=[.3, .7]).reshape(n, n)
...
def update_grid(frame_num, img, grid: np.array, n: int, germ_grid: np.array, germ_cells, genes):
    ...
    with ThreadPoolExecutor() as executor:
        results = list(executor.map(update_sub_grid, sub_grids))  # process each sub-grid in parallel
...
def update_sub_grid(args):
    ...
    for i in range(i_start, i_end):
        for j in range(j_start, j_end):
            total = int((np.sum(local_grid[(i+neighbor_offsets[:, 0]) % n, (j+neighbor_offsets[:, 1]) % n]) - local_grid[i, j]) / ON)
