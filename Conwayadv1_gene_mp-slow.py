...
def update_grid(frame_num, img, grid: np.array, n: int, germ_grid: np.array, germ_cells, genes):
    ...
    new_germ_cells = []
    pool = mp.Pool()  # create a pool of worker processes
    sub_grid_size = 10  # set the size of each sub-grid
    sub_grids = [(local_grid, germ_grid, germ_cells, genes, i, j, n, sub_grid_size) for i in range(0, n, sub_grid_size) for j in range(0, n, sub_grid_size)]
    # divide the grid into sub-grids of size sub_grid_size x sub_grid_size
    results = pool.map(update_sub_grid, sub_grids)  # process each sub-grid in parallel
    pool.close()
    pool.join()
    for res in results:
        local_grid[res[0]:res[1], res[2]:res[3]] = res[4]
        local_germ_grid[res[0]:res[1], res[2]:res[3]] = res[5]
        new_germ_cells.extend(res[6])
...
def update_sub_grid(args):
    local_grid, germ_grid, germ_cells, genes, i_start, j_start, n, sub_grid_size = args
    i_end = min(i_start + sub_grid_size, n)
    j_end = min(j_start + sub_grid_size, n)
    new_germ_cells = []
    for i in range(i_start, i_end):
        for j in range(j_start, j_end):
            total = int(((local_grid[i, (j-1) % n] + local_grid[i, (j+1) % n]) + local_grid[(i-1) % n, j] +
                         local_grid[(i+1) % n, j] + local_grid[(i+1) % n, (j+1) % n] + local_grid[(i-1) % n, (j-1) % n] +
                         local_grid[(i+1) % n, (j-1) % n] + local_grid[(i-1) % n, (j+1) % n])/ON)
            if local_grid[i, j] == ON and (total < 2 or total > 3):
                local_grid[i, j] = OFF
            elif total == 3:
                new_state, offspring_genes = germ_rule(local_grid, germ_grid, germ_cells, i, j)
                ...
                new_state, offspring_genes = germ_rule(local_grid, germ_grid, germ_cells, i, j)
                local_grid[i, j] = new_state
                germ_grid[i, j] = germ_grid[i, j]
                if germ_grid[i, j] == ON:
                    matching_cells = [(cell[2], cell[0], cell[1]) for cell in germ_cells if cell[0] == i and cell[1] == j]
                    if matching_cells:
                        parent_genes, _, _ = matching_cells[0]
                    else:
                        parent_genes = offspring_genes
                        offspring_genes = [random.gauss(mu=genes[k], sigma=0.1) for k in range(len(genes))]
                    germ_cells.append((i, j, offspring_genes))
            else:
                germ_grid[i, j] = OFF
    return i_start, i_end, j_start, j_end, local_grid[i_start:i_end, j_start:j_end], germ_grid[i_start:i_end, j_start:j_end], new_germ_cells
